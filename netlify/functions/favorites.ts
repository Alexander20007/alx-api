...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (action === 'add') {
            if (!mediaId || !mediaType || !title) return { statusCode: 400, body: JSON.stringify({ error: 'mediaId, mediaType e title são obrigatórios' }) };
        ...
        if (action === 'check') {
            ...
            return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ success: true, isFavorite: isFav }) };
        }
        return { statusCode: 400, body: JSON.stringify({ error: 'Ação inválida. Use get, add, remove ou check' }) };
    } catch (error: any) {
