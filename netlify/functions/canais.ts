...
export const handler: Handler = async (event) => {
    ...
    try {
        if (action === 'grupos') {
            ...
            return { statusCode: 200, headers, body: JSON.stringify({ success: true, total: grupos.length, grupos }) };
        }
        const limitNum = limit ? parseInt(limit) : 100;
        const resultado = await buscarCanais(query, grupo, limitNum);
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, ...resultado }) };
    } catch (error: any) {
