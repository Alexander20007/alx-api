...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (action === 'update') {
            if (!mediaId || !mediaType || progress === undefined) return { statusCode: 400, body: JSON.stringify({ error: 'mediaId, mediaType e progress são obrigatórios' }) };
        ...
        return { statusCode: 400, body: JSON.stringify({ error: 'Ação inválida. Use get, update ou delete' }) };
    } catch (error: any) {
